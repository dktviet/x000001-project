...
<? }else{
	...
	for($i=0; $i< $a; $i++){	
		...
		$pathdb = "images/gallery";
		
		$path_thumb = "../images/gallery/thumbs";
		if(!is_dir($path_thumb)){ mkdir($path_thumb,0777);}
		$pathdb_thumb = "images/gallery/thumbs";

		@chmod("$path/".$_POST['fileimg'][$i], 0777);
		change_img_size("$path/".$_POST['fileimg'][$i],"$path/".$_POST['fileimg'][$i],800,800);
		...
		$sql = "insert into  ".tbl_config::tbl_content."  (name_vn, name_en, parent, image, image_large, sort, status,  date_added, last_modified) values ('".$name_vn.'-'.$i."','".$name_en.'-'.$i."','".$parent."','".$img_thumb_db."','".$img_db."','".$sort."','".$status."',now(),now())";
